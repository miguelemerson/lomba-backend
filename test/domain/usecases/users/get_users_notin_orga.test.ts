...
import {GetUsersNotInOrga} from '../../../../src/domain/usecases/users/get_users_notin_orga';
import { MockUserRepository } from './user_repository.mock';
describe('Conseguir usuarios que no estén en una organización - Caso de uso', () => {
	...
	test('el usecase de conseguir usuarios que no esté en una orga debe retornar ok', async () => {
		//arrange
		jest.spyOn(mockUserRepository, 'getUsersNotInOrga').mockImplementation(() => Promise.resolve(Either.right(new ModelContainer<UserModel>(listUsers))));
	...
	test('el usecase de conseguir usuario no asociado a orga debe retornar failure', async () => {
		//arrange
		jest.spyOn(mockUserRepository, 'getUsersNotInOrga').mockImplementation(() => Promise.resolve(Either.left(new GenericFailure('error'))));
