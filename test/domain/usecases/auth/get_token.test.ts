...
describe('Autenticar usuario - Caso de uso', () => {
	...
	test('el usecase de autenticación debe retornar ok', async () => {
		//arrange
		jest.spyOn(mockAuthRepository, 'getAuth').mockImplementation(() => Promise.resolve(Either.right(ModelContainer.fromOneItem(tokenModel))));

		//act
		const useCase = new GetToken(mockAuthRepository);
		const result = await useCase.execute(testAuth);
		//assert
		expect(mockAuthRepository.getAuth).toBeCalledTimes(1);
	...
	test('el usecase de autenticación de usuario debe retornar failure', async () => {
		//arrange
		jest.spyOn(mockAuthRepository, 'getAuth').mockImplementation(() => Promise.resolve(Either.left(new GenericFailure('error'))));
