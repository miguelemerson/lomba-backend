...
import { GetOrgasByUser } from '../../../../src/domain/usecases/orgas/get_orgas_by_user';
...
describe('Conseguir usuario por orga - Caso de uso', () => {
	...
	test('el usecase de conseguir usuario por orga debe retornar ok', async () => {
		//arrange
		jest.spyOn(mockOrgaUserRepository, 'getOrgasByUserId').mockImplementation(() => Promise.resolve(Either.right(new ModelContainer<Orga>(listOrgas))));

		//act
		const useCase = new GetOrgasByUser(mockOrgaUserRepository);
		const result = await useCase.execute('u');
		//assert
		expect(mockOrgaUserRepository.getOrgasByUserId).toBeCalledTimes(1);
	...
	test('el usecase de conseguir usuario por orga debe retornar failure', async () => {
		//arrange
		jest.spyOn(mockOrgaUserRepository, 'getOrgasByUserId').mockImplementation(() => Promise.resolve(Either.left(new GenericFailure('error'))));
