...
describe('Conseguir usuario por orga - Caso de uso', () => {
	...
	test('el usecase de conseguir usuario por orga debe retornar ok', async () => {
		//arrange
		jest.spyOn(mockOrgaRepository, 'getOrgas').mockImplementation(() => Promise.resolve(Either.right(new ModelContainer<OrgaModel>(listOrgas))));
		...
		expect(result).toEqual(Either.right(new ModelContainer<OrgaModel>(listOrgas)));
	});
