...
export interface UpdateOrgaUserUseCase {
    execute(orgaId:string, userId:string, orgaUser: OrgaUserModel): Promise<Either<Failure,ModelContainer<OrgaUser>>>;
}
...
export class UpdateOrgaUser implements UpdateOrgaUserUseCase {
	...
	async execute(orgaId:string, userId:string, orgaUser: OrgaUserModel): Promise<Either<Failure,ModelContainer<OrgaUser>>> {
