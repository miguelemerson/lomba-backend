...
export default function UsersRouter(
	...
	router.get('/:id', async (req: Request, res: Response) => {
		...
		try {
			...
			{
				code = 404;
				toSend = new RouterResponse('1.0', new Error('Not found'), 'get', {id: req.params.id} as object, 'user not getted');
			} else
			{
				code = 200;
	...
	router.get('/byorga/:orgaId', async (req: Request<{orgaId:string}>, res: Response) => {
		try {
			...
			res.status(500).send(new RouterResponse('1.0', err as object, 'get', {orgaId: req.params.orgaId} as object, 'not obtained by orga id'));
		}
	});
	...
	router.post('/', async (req: Request, res: Response) => {
		...
		try {
			...
			res.status(500).send(new RouterResponse('1.0', err as object, 'post', undefined, 'user was not added'));
		}
	});
	...
	router.put('/:id', async (req: Request, res: Response) => {
		try {
			...
			res.status(500).send(new RouterResponse('1.0', err as object, 'put', {id: req.params.id}, 'user was not edited'));
		}
	});
	...
	router.put('/enable/:id', async (req: Request<{id:string, enable:boolean}>, res: Response) => {
		...
		try {
			...
			res.status(500).send(new RouterResponse('1.0', err as object, 'put', {id: req.params.id, enable: req.query.enable}, 'user was not ' + text));
		}
	});
	...
	router.delete('/:id', async (req: Request<{id:string}>, res: Response) => {
		try {
			...
			res.status(500).send(new RouterResponse('1.0', err as object, 'delete', {id: req.params.id}, 'user was not deleted'));
		}
	});    
